...
int main(int argc, char* argv[]) {
    try {
        std::cerr << "HDLCd pcap streamer for HDLC frames\n";
        if (argc != 4) {
            std::cerr << "Usage: hdlcd-pcapstreamer-hdlc <host> <port> <usb-device>\n";
            return 1;
        } // if
        ...
        HdlcdAccessClient l_AccessClient(io_service, endpoint_iterator, argv[3], 0x33);
